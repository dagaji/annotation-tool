...
def get_section(section_name, step_msec):
	...
	return range(utils.string2msec(section_start), utils.string2msec(section_end) + step_msec, step_msec)
...
class MaskExtractor:
	...
	def extract(self, full_mask, parameters, section):
		...
		assert len(section) == len(Minv_list) == len(coords1_list) == len(coords2_list), "Could not extract masks"
